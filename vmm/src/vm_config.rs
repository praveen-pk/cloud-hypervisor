...
impl DeviceConfig {
    pub fn apply_landlock(self, mut landlock: RefMut<Landlock>) -> LandlockResult<()> {
        ...
        let device_path = fs::read_link(self.path).map_err(LandlockError::OpenDevicePath)?;
...
impl VmConfig {
    pub fn apply_landlock(&self, landlock: Rc<RefCell<Landlock>>) -> LandlockResult<()> {
        ...
        if let Some(devices) = self.devices.as_ref() {
            let vfio_dev_flags = landlock::READ | landlock::WRITE;
            landlock
                .borrow_mut()
                .add_rule_with_flags("/dev/vfio/vfio".into(), vfio_dev_flags)?;
